package TemplateProject.Template.global.security.auth.handler;

import TemplateProject.Template.global.exception.dto.ErrorResponse;
import TemplateProject.Template.global.exception.exceptionCode.ExceptionCode;
import com.google.gson.Gson;
import org.springframework.security.core.AuthenticationException;
import org.springframework.security.web.AuthenticationEntryPoint;
import org.springframework.stereotype.Component;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;

@Component
public class AccountAuthenticationEntryPoint implements AuthenticationEntryPoint {

    @Override
    public void commence (HttpServletRequest request,
                          HttpServletResponse response,
                          AuthenticationException excaption) throws IOException, ServletException {
        Gson gson = new Gson();
        ExceptionCode exceptionCode = ExceptionCode.ACCESS_TOKEN_EXPIRATION;
        ErrorResponse errorResponse = new ErrorResponse(exceptionCode.getStatus(), "BusinessLoginException", exceptionCode.getMessage());
        response.setContentType("application/json");
        response.setStatus(exceptionCode.getStatus());
        response.getWriter().write(gson.toJson(errorResponse, ErrorResponse.class));
    }

}
